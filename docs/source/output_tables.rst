...
| In addition, OCTOPUS CRN collections also provides :ref:`CRN_ancillary_data` comprising **raster layers**, including a *digital elevation model*, *gradient raster*, *flow direction* and *flow accumulation rasters*, *atmospheric pressure raster*, and *CRN production scaling* and *topographic shielding factor* rasters, and a set of **structured data files** (see :ref:`CSV_folder` subsection) for data recalculation integrity.
